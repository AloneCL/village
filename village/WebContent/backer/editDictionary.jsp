...
<article class="page-container">
	<form action="submitEditDctionary.action" method="post" class="form form-horizontal" id="form-member-add">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">
			<input type="text" name="id" value="${dictionary.id }" hidden>
			<span class="c-red">*</span>名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${dictionary.name }" placeholder="" id="name" name="name">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
			    ...
			    <option value="0" <c:if test="${dictionary.type == 0}">selected</c:if>>粮食</option>
			    <option value="1" <c:if test="${dictionary.type == 1}">selected</c:if>>蔬菜</option>
			    <option value="2" <c:if test="${dictionary.type == 2}">selected</c:if>>水果</option>
			    <option value="3" <c:if test="${dictionary.type == 2}">selected</c:if>>牲畜</option>
		</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="大概成熟需要的月数" name="growTime" id="growTime" value="${dictionary.growTime }" onkeypress="return event.keyCode>=48&&event.keyCode<=57" ng-pattern="/[^a-zA-Z]/">
			</div>
		</div>
		<div class="row cl">
		    ...
		    <input type="button" class="btn btn-primary radius tableButton modityImg" value="设置封面" id="uploadButton"/>
			<img class="tableImg" src="${dictionary.imgURL }" width="180" height="110" name="flag"/>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="introduce" cols="" rows="" class="textarea" placeholder="说点什么...最少输入10个字符" onKeyUp="$.Huitextarealength(this,100)">${dictionary.introduce } </textarea>
