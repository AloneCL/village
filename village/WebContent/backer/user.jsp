	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				onclick="user_add('添加用户','submitEditUser.action','','510')"
					...
					<c:forEach items="${users}" var="user">
						<tr class="text-c">
							...
							<c:if test="${user.userType == 1}">
								<td class="user-status"><span class="label label-success">买家</span></td>
							</c:if>
							<td>${user.starLevel}</td>
							<td class="td-manage"><a title="编辑" href="javascript:;"
								onclick="user_edit('编辑','editUser.action?id=${user.id}')"
								class="ml-5" style="text-decoration: none"><i
									class="Hui-iconfont">&#xe6df;</i></a> <a title="删除"
								href="javascript:;"
								onclick="user_del(this,'deleteUser.action?id=${user.id}')"
								class="ml-5" style="text-decoration: none"><i
		...
		function datadel(){
		    ...
		    if(check_val!=0){
		    	layer.confirm('确认要删除吗？', function(index) {
					$.ajax({
						type : 'POST',
						url : 'deleteManyUser.action?id='+check_val,
						success : function() {
							layer.msg('已删除!', {
								icon : 1,
								time : 2000
